...
function makeLines(draw, bg, circles) {
    ...
    return (
      draw
        ...
        .on("redraw", function () {
          ...
          this.plot(c1.cx(), c1.cy(), c2.cx(), c2.cy()).stroke(
            this.data("crossed") ? "red" : "green"
          );
        })
  ...
  function linesIntersect(l1, l2) {
    ...
    function samePoint(x0, y0, x1, y1) {
      const allowance = 10;
      return Math.abs(x0 - x1) < allowance && Math.abs(y0 - y1) < allowance;
    }

    // check to see if three points are counterclockwise
    function CCW(x0, y0, x1, y1, x2, y2) {
      return 0 > Math.sign(cross(x1 - x0, y1 - y0, x2 - x0, y2 - y0));
    }
    ...
    const crossed =
      ...
      CCW(x0, y0, x2, y2, x3, y3) !== CCW(x1, y1, x2, y2, x3, y3) &&
      CCW(x2, y2, x0, y0, x1, y1) !== CCW(x3, y3, x0, y0, x1, y1);
  ...
  const w_min = draw.data("w_min"),
    ...
    cw = (w_min + w_max) / 2,
    ch = (h_min + h_max) / 2,
    r = Math.min((w_max - w_min) / 2, (h_max - h_min) / 2);
